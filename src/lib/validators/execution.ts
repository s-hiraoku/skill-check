...
export function validateExecution(skill: ParsedSkill): CheckResult {
  ...
  if (hasWhenToUse) {
    score += 6;
    issues.push({ code: "EXEC_WHEN_TO_USE", message: "Has 'When to Use' trigger section", severity: "info" });
  } else {
    issues.push({
      code: "EXEC_NO_WHEN_TO_USE",
  ...
  if (actionableCount >= 3) {
    score += 6;
    issues.push({ code: "EXEC_ACTIONABLE", message: "Content has structured, actionable sections", severity: "info" });
  } else {
    issues.push({ code: "EXEC_UNSTRUCTURED", message: "Limited structure detected; add headings, lists, or code blocks", severity: "warning" });
    score += 3;
  }
  ...
  if (hasExamples) {
    score += 4;
    issues.push({ code: "EXEC_EXAMPLES", message: "Includes examples section or references", severity: "info" });
  } else {
    issues.push({ code: "EXEC_NO_EXAMPLES", message: "No examples found — consider adding usage examples", severity: "info" });
    score += 2;
  }
  ...
  if (hasScripts) {
    score += 4;
    issues.push({ code: "EXEC_SCRIPTS", message: "Contains executable script references or code blocks", severity: "info" });
  } else {
    score += 2;
  }
  ...
  return {
    ...
    maxScore,
    issues,
    details: { hasWhenToUse, actionableCount, hasExamples, hasScripts },
  };
}
